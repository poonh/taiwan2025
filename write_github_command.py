...
with open("github_command.txt","w") as gitfile:
     C = f'git add {folder}\n'
     D = f'git commit -m "Add {folder} with initial files"\n'
     E = f'git push origin main\n'
   
     gitfile.write(C+D+E)
     jpg_files = [f for f in os.listdir(f'{folder}') if f.endswith('.jpg')]
     count = len(jpg_files)
     for jpg in jpg_files:
           a = f'git add --force {folder}/{jpg}\n'
           b = f'git commit -m "Upload {folder}/{jpg}"\n'
